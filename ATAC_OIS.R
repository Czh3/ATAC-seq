...
atac = read.table("/lustre/user/liclab/zhangc/Taolab/xiaohuangli/OIS/ATAC_new/peaks/merge/atac.merge.reads.bed")
rownames(atac) = paste(atac$V1, atac$V2, atac$V3, sep="\t")
atac = atac[,5:10]
colnames(atac) = c("B_pro", "B_ras", "D_pro", "D_ras", "E_pro", "E_ras")
condition = c("proliferation", "Ras", "proliferation", "Ras", "proliferation", "Ras")
...
boxplot(logcpm)
# MA-plot
ggplot(data.frame(peaks_cpm)[1:6000, ], aes(log(B_pro) + log(B_ras), log(B_pro) - log(B_ras))) +
...
row.names(condition.info) =  c("B_pro", "B_ras", "D_pro", "D_ras", "E_pro", "E_ras")
pca = prcomp(t(peaks_cpm), scale = T)
